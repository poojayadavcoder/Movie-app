  ...
  return (
    <>
      ...
      <div className="w-full min-h-[100vh] bg-black relative">
        ...
        <div
          className="w-full min-h-[500px] flex justify-between
     items-center gap-[40px] md:gap-0 md:items-start flex-col md:flex-row"
        >
          <div
            className="w-[280px] h-[300px]
       relative md:z-20 mt-[100px] mb-[30px] md:mt-[-90px] md:ml-5 rounded-[12px]"
          >
            <div
              className="w-full h-[300px] rounded-[5px]  relative
         overflow-hidden"
            >
              {movie.images ? (
                <Image
            ...
            <button
              onClick={() => handleWatchNow(movie.watchUrl)}
              className="border-[1px] border-violet-400 rounded-[8px] text-[14px] w-full text-violet-400 px-7 py-[6px] mt-4 cursor-pointer
          hover:bg-violet-400 transition-all duration-150 ease-linear hover:text-black"
            >
              Watch Now
            </button>
          </div>
          ...
          <div className="w-[100%] min-h-[300px] mt-5 flex items-center justify-end">
            <div
              className="w-full md:w-[95%] min-h-[300px]
        flex justify-start items-start flex-col gap-2"
            >
              <h1 className="text-violet-400 text-[17px] pl-2 md:pl-0">
                {movie.title}
              </h1>
              <div className="flex pl-2 md:pl-0">
                {movie.genre
                  ? movie.genre.map((item, index) => {
                      return (
                        <p className="text-white text-[14px]" key={index}>
                          {item} {index == 2 ? "" : "|"}&nbsp;
                        </p>
                      );
                    })
              ...
              {movie.description ? (
                <p className="text-white text-[14px] pl-2 md:pl-0">
                  {movie.description}
                </p>
              ) : (
                ""
              ...
              <div className="flex w-full gap-5 pl-2 md:pl-0">
                {movie.year ? (
                  <div className=" flex justify-center items-center gap-[4px]">
                    ...
                    <p className="text-white text-[14px]">{movie.year}</p>
                  </div>
                ) : (
                  <p className="text-white text-[14px]">2024</p>
                )}
                {movie.rating ? (
                  <div className="flex justify-center items-center gap-[4px]">
                    <span className="text-violet-400 text-[15px]">
                    ...
                    <p className="text-white text-[14px]">{movie.rating}</p>
                  </div>
                ) : (
                  <p className="text-white text-[14px]">8.3</p>
                )}
                {movie.duration ? (
                  <div className="flex justify-center items-center gap-[4px]">
                    <span className="text-violet-400 text-[15px]">
                    ...
                    <p className="text-white text-[14px]">{movie.duration}</p>
                  </div>
                ) : (
                  <p className="text-white text-[14px]">3h 20min</p>
                )}
              </div>
             ...
             flex-col gap-5"
              >
                <h1 className="text-xl font-semibold text-white pl-2 md:pl-0">
                  Cast
                </h1>
                <div
                ...
                >
                  {movie.cast
                    ? movie.cast.map((item, index) => {
                        return (
                          <div
                            className="w-[140px] min-h-[140px] flex justify-center items-center flex-col gap-5"
                            key={index}
                          >
                            <div
                            ...
                            <h1 className="text-[13px] font-semibold text-white text-center">
                              {item.name}
                            </h1>
                          </div>
                        );
      ...
      <div
        className="w-full h-auto px-3 flex justify-start 
            ...
            {allMovies.map((movie) => (
              <SwiperSlide key={movie.id}>
                ...
                >
                  ...
                  <h1
                    className="absolute bg-black bottom-[0px] left-0 border-[1px] border-t-0 border-x-0 border-b-violet-400
